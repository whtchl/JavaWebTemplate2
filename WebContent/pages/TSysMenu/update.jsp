...
<HTML xmlns="http://www.w3.org/1999/xhtml"> 
		...
		<script type="text/javascript">
			$(function () {
	     		 $.ajax({
					type : 'post',
					dataType : 'json',
					url:'../../tsysmenu/select.action',
					data:{'menuId':${param.pkName}},
					error : function() {// 请求失败处理函数
						alert('请求失败');
					},
					success : function(data) { // 请求成功后处理函数。
	                	var inputValArray=$('#TSysMenuUpdateForm').serialize().split('&');
						for(var i=0;i<inputValArray.length;i++){
						     var inputName=inputValArray[i].split('=')[0];
						     $("#TSysMenuUpdateForm [name='"+inputName+"']").val(row_info(data[0],inputName));
						}
					}
				});
	     	});
	...
	<BODY>   
		...
		<div class="easyui-layout" data-options="fit:true">
			<form id="TSysMenuUpdateForm" method="post" > 
				...
				<div data-options="region:'south',border:false" style="text-align:right;padding:10px;padding-bottom: 5px;" >
					<a class="easyui-linkbutton" data-options="iconCls:'icon-ok'" href="javascript:void(0)" onclick="update_save('TSysMenuUpdateForm','tsysmenu')">修改</a>
				</div>
			</form>
		</div>
