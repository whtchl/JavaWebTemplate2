$(function (){
			    ...
			    onClick: function(node){
			     ...
			     $("#add_tsysmenu").attr("style","display:none");
			        $.post("../../tsysmenu/selectPage.action?idArray="+node.id,function(data){
			                   ...
			                   //设置显示父级
			   				   parent=$('#jKTreeMenu').tree('getParent',node.target);
			   				   if(parent!=null){
				                   $("#up_parentid").val(parent.id);
				                   $("#up_parentName").val(parent.text);
			   				   }else{
				                   $("#up_parentid").val(0);
				                   $("#up_parentName").val("");
			   				   }
    ...
    function menuValue(){
		...
		//添加下级
		function tsysmenu_add_next(){
			var node = selectNode();
			$("#up_parentid").val(node.id);
			$("#up_parentName").val(node.text);
	        $("#up_menuHref").removeAttr("readonly");
			initialData(node, 1);//设置初始值 ，添加同级下级时，【级别，名称，编号】
		}
		//添加同级
		function tsysmenu_add_this(){
			var node = selectNodeParent();
	        $("#up_menuHref").removeAttr("readonly");
			if(node!=null){
				$("#up_parentid").val(node.id);
				$("#up_parentName").val(node.text);
...
//				$("#up_menuHref").val("");
		...
		//设置初始值 ，添加同级下级时，【级别，名称，编号】
		function initialData(node, i){
			$.post("../../tsysmenu/selectPage.action?idArray="+node.id,function(data){
				var menuLevel = data[0].menuLevel;
				$("#up_menuLevel").val(menuLevel+i);
			},'json')
		...
		//当前选中树节点
		function selectNode(){
			var node = $('#jKTreeMenu').tree('getSelected');
			return node;
		}
		//当前选中树节点的父节点
		...
		function tsysmenu_delete(){
			var actionName;
	        var menuId = $("#up_menuId").val();
			...
			if(menuId==''){
				messager('信息提示：请选择删除菜单的信息!!!'); 
			}else{
				$.messager.confirm('删除提示', "你确定删除id是:&nbsp;&nbsp;"+menuId+"&nbsp;&nbsp;&nbsp;的数据信息吗?", function(r){
					if (r){
						$.post("../../"+actionName+"/detele.action?idArray="+menuId,function(data){
							if(data==1){
...
//	       }
			...
			$.ajax({
				...
				success : function(data) { // 请求成功后处理函数。
					if (data ==1) {
						//重置所有文本框的值
						messager("提交成功,可以继续操作");
						$("#"+divName+"").tree('reload');
						document.getElementById("menuForm").reset(); 
					}else{
						messager("提交失败,请查看修改的信息是否正确!!!");
					}
				}
			});
		}
