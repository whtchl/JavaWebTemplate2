...
public class ReadTable {
   ...
   /**
	...
	public Map read(String table) {
		...
		try {
			...
			for (int i = 1; i < rsmd.getColumnCount() + 1; i++) {
			    ...
			    System.out.println(rsmd.getColumnName(i).toLowerCase()+" "+rsmd.getColumnTypeName(i).toLowerCase()+" "+rsmd.getScale(i)+"");
			}
	...
	public int findDecimals(String table,String attribute){
		String sql = "SELECT nvl(data_scale,0) FROM ALL_TAB_COLUMNS WHERE OWNER= (select default_tablespace from user_users) and data_type = 'NUMBER' and table_name = '"+table+"'  and column_name = '"+attribute+"' ";
		Connection conn = db.getConn();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int num = 0;
...
//		String findsql = "select column_name,nvl(comments,'') from user_col_comments where table_name='"+table.toUpperCase()+"'"; 
		String findsql = "SELECT COLUMN_NAME,COLUMN_COMMENT FROM information_schema.COLUMNS WHERE table_name = '"+table+"'"; 
...
//		String sql="select  col.column_name as name from user_constraints con,user_cons_columns col where con.constraint_name=col.constraint_name and con.constraint_type='P' and col.table_name='"+table_name.toUpperCase()+"'";
		String sql="SELECT COLUMN_NAME NAME FROM information_schema.COLUMNS WHERE table_name = '"+table_name+"' AND COLUMN_KEY = 'PRI'";
